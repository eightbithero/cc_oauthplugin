...
class sfOAuth2Server extends OAuth2 {
	...
	$q = sfOauthServerConsumerQuery::create()->findOneByConsumerKey($client_id);
	if (!$q)
		return false;
	else
  ...
  protected function getAccessToken($oauth_token) {
 	$q = sfOauthServerAccessTokenQuery::create()->findOneByToken($oauth_token);
	if ($q)
		{
  ...
  protected function setAccessToken($oauth_token, $client_id, $expires, $scope = NULL) {
		...
		$consumer = sfOauthServerConsumerQuery::create()->findOneByConsumerKey($client_id);
		if (!$consumer)
			throw new OAuthException('Invalid Request');
