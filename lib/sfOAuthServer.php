...
class sfOAuthServer extends OAuthServer {
	...
	{
		$sfToken = sfOauthServerRequestTokenQuery::create()->findOneByToken($request->getParameter('oauth_token'));
		if (!$sfToken)
				return false;	
		return true;
	}
	...
	{
		$sfToken = sfOauthServerRequestTokenQuery::create()->findOneByToken($token);
		$sfToken->setUserId($userId);
		$sfToken->save();
		
		SfOauthServerUserScopeQuery::create()->authorizeApplication($sfToken->getConsumerId(),$userId,$sfToken->getScope());
	}
...
class sfOAuthDataStore extends OAuthDataStore {
    ...
    function lookup_consumer($consumer_key) {/*{{{*/
		$q = sfOauthServerConsumerQuery::create()->findOneByConsumerKey($consumer_key);
		if ($q)
		{
    ...
    function lookup_token($consumer, $token_type, $token) {
		if ($token_type=='request')
			$q = sfOauthServerRequestTokenQuery::create()->findOneByToken($token);
		else if ($token_type=='access')
			$q = sfOauthServerAccessTokenQuery::create()->findOneByToken($token);
		if ($q)
		{
    ...
    function lookup_nonce($consumer, $token, $nonce, $timestamp) {
		$q = sfOauthServerNonceQuery::create()->findOneByNonce($nonce);
		if ($q)
			return true;
		else
		{
  ...
  function new_request_token($consumer, $callback = null) {
		...
		$c = sfOauthServerConsumerQuery::create()->findOneByConsumerKey($consumer->key);
  ...
  function new_access_token($token, $consumer, $verifier = null) {
		...
		$c = sfOauthServerConsumerQuery::create()->findOneByConsumerKey($consumer->key);
		$token = sfOauthServerRequestTokenQuery::create()->findOneByToken($token->key);
		$key = md5(time());
		$secret = time() + time();
